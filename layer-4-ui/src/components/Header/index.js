/**
 * Header/index.js — верхняя панель.
 *
 * Путь проекта берётся из store.projectDir (устанавливается wsClient через server:status)
 */
...
const PENCIL_SVG = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`;
const PLUG_SVG   = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 22v-5"/><path d="M9 8V2"/><path d="M15 8V2"/><path d="M18 8H6a2 2 0 0 0-2 2v3a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2z"/></svg>`;
const WS_LABEL = {
    connected:    'активен',
...
export class Header {
    ...
    _bindEvents() {
        ...
        this.unsubs.push(
            on('ws:status', () => this._syncWsStatus()),
            on('graph:full',   () => { this._syncProject(); this._syncUpdatedAt(); }),
            on('graph:update', () => this._syncUpdatedAt()),
            on('project:status', () => this._syncProject()),
            on('graph:rebuild:done', (updatedAt) => {
                if (updatedAt > 0) this._setUpdatedAt(updatedAt, true);
            }),
            on('theme:changed', () => this._syncThemeIcon()),
            on('project:changed', () => this._syncProject()),
        );
    }
    _syncWsStatus() {
    ...
    /**
     * Путь проекта берётся из store.projectDir (устанавливается через server:status).
     * Если нет — показываем кнопку «Подключить».
     */
    _syncProject() {
        const projectPath = store.projectDir;
        this._projectZone.innerHTML = '';
        if (projectPath) {
            // Показываем только последний сегмент пути + полный путь в title
            const shortPath = projectPath.split(/[\\/]/).filter(Boolean).slice(-2).join('/');
            ...
            editBtn.setAttribute('title', projectPath);
    ...
    _setUpdatedAt(ts, flash) {
        if (!ts || ts === 0) { this._updatedEl.textContent = ''; return; }
        const d  = new Date(ts);
    ...
    async _doRebuild() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (raw) { const n = Number(raw); if (Number.isFinite(n) && n > 0) updatedAt = n; }
                try {
