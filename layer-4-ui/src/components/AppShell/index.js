...
import { syncGraph, updateTheme, highlightSelected, clearDataflowHighlight, applyDataflowHighlight, startDashAnimation, stopDashAnimation, } from '../../graph/cytoscapeInit.js';
...
export class AppShell {
    ...
    _bindEvents() {
        // ---- cy:ready
        this.unsubs.push(on('cy:ready', (cyInstance) => {
            this.cy = cyInstance;
        }));
        // ---- graph:full — полный снимок пришёл по WS
        this.unsubs.push(on('graph:full', (graph) => {
            if (this.cy)
        ...
        this.unsubs.push(on('graph:update', (_payload) => {
            if (this.cy)
        ...
        this.unsubs.push(on('graph:refresh', async () => {
            try {
                ...
                store.setGraph(data);
                if (this.cy)
                    syncGraph(this.cy, data, store.theme === 'dark');
                this.sidebar.update(store.graph);
                this.header.update();
            }
            catch (err) {
        ...
        // ---- graph:rebuild:done
        this.unsubs.push(on('graph:rebuild:done', (_updatedAt) => {
            this.header.update();
        }));
        ...
        this.unsubs.push(on('project:changed', (_dir) => {
            disconnectWs();
            setTimeout(() => {
                connectWs();
            }, 1000);
        ...
        // ---- node:select
        this.unsubs.push(on('node:select', (id) => {
            store.selectNode(id);
        ...
        // ---- node:deselect
        this.unsubs.push(on('node:deselect', () => {
        ...
        // ---- dataflow:toggle
        this.unsubs.push(on('dataflow:toggle', (enabled) => {
        ...
        // ---- dataflow:next
        this.unsubs.push(on('dataflow:next', () => {
        ...
        // ---- theme:changed
        this.unsubs.push(on('theme:changed', (theme) => {
        ...
        // ---- sidebar:collapsed
        this.unsubs.push(on('sidebar:collapsed', (collapsed) => {
            const sidebarEl = this.root.querySelector('.app-sidebar');
            if (sidebarEl)
