...
export const DATAFLOW_NODE_IDS: Record<DataflowPathIndex, string[]> = {
...
function injectStyles(): void {
  if (document.getElementById('df-mode-css')) return;
...
function applyHighlight(cy: Core, pathIndex: DataflowPathIndex): void {
  ...
  cy.batch(() => {
    ...
    cy.nodes().forEach((n: NodeSingular) => {
      if (nodeIds.has(n.id())) n.addClass('df-active');
      else                     n.addClass('dimmed');
    });

    cy.edges().forEach((e: EdgeSingular) => {
      const srcIn = nodeIds.has(e.data('source') as string);
      const tgtIn = nodeIds.has(e.data('target') as string);
      if (srcIn && tgtIn) e.addClass('df-active');
...
export const DataFlowMode = {
  _cy:       null as Core | null,
  _selector: null as HTMLElement | null,
  _buttons:  [] as HTMLButtonElement[],
  ...
  mount(parent: HTMLElement): void {
    ...
    const paths: [DataflowPathIndex, string, number][] = [
    ...
    paths.forEach(([idx, label, count], i) => {
      if (i > 0) {
        sel.appendChild(
          Object.assign(document.createElement('div'), { className: 'df-path-sep' })
        );
      }
  ...
  _selectPath(idx: DataflowPathIndex): void {
  ...
  _syncButtons(): void {
    ...
    this._buttons.forEach(btn => {
      const idx = Number(btn.dataset.idx) as DataflowPathIndex;
  ...
  _startDash(): void {
    if (this._timer || !this._cy) return;
    this._offset = 0;
    const cy = this._cy;
