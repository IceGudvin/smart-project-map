...
function inject() {
  if (document.getElementById('zoom-controls-css')) return;
  const s = document.createElement('style'); s.id = 'zoom-controls-css';
...
export const ZoomControls = {
  mount(parent: HTMLElement) {
    ...
    const mkBtn = (label: string, title: string, ev: string) => {
      const b = document.createElement('button');
      b.className = 'zoom-btn';
      b.title = title;
      b.setAttribute('aria-label', title);
      b.innerHTML = label;
      b.addEventListener('click', () => emit(ev as any, null));
      return b;
    };
