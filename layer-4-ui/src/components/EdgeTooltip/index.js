...
function injectStyles() {
    if (document.getElementById('edge-tooltip-css'))
        return;
...
export const EdgeTooltip = {
    ...
    mount() {
        injectStyles();
        if (document.getElementById('edgeTooltip'))
            return; // уже есть
        this._el = createEl();
        ...
        window.addEventListener('mousemove', (e) => {
            if (this._visible)
                this._moveToClient(e.clientX, e.clientY);
        }, { passive: true });
    },
    _show(p) {
        const el = this._el;
