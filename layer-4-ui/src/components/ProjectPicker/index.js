...
function inheritTheme(el) {
    ...
    applyTheme();
    return on('theme:changed', (theme) => applyTheme(theme));
}
// ───────────────────────── styles
...
let _fbCurrentPath = '';
let _fbSelectedPath = '';
let _fbOnSelect = null;
let _fbThemeUnsub = null;
...
function _fbRender(result) {
    const topbar = _fbOverlay.querySelector('.fb-topbar');
    const list = _fbOverlay.querySelector('.fb-list');
    const selectedPathEl = _fbOverlay.querySelector('.fb-selected-path');
    const btnSelect = _fbOverlay.querySelector('.fb-btn-select');
    const btnUp = _fbOverlay.querySelector('.fb-btn-up');
    _fbCurrentPath = result.path;
    const breadcrumb = topbar.querySelector('.fb-breadcrumb');
    breadcrumb.innerHTML = result.breadcrumbs.map((c, i) => `<span class="fb-crumb" data-path="${c.path}">${c.label}</span>` +
        (i < result.breadcrumbs.length - 1 ? `<span class="fb-crumb-sep">›</span>` : '')).join('');
    ...
    btnUp.onclick = () => { if (result.parent)
        _fbNavigate(result.parent); };
    list.innerHTML = '';
...
function _showFileBrowser(initialPath, onSelect) {
    if (_fbOverlay)
        return;
    _fbSelectedPath = '';
...
function _render() {
  ...
  `;
    _input = overlay.querySelector('.pp-input');
    _statusEl = overlay.querySelector('.pp-status');
    _goBtn = overlay.querySelector('.pp-btn-open');
    const browseBtn = overlay.querySelector('.pp-btn-browse');
    overlay.querySelectorAll('.pp-recent-item').forEach((li) => {
        li.addEventListener('click', () => {
            if (_input)
                _input.value = li.dataset.path ?? '';
            _submit();
        });
    });
    browseBtn.addEventListener('click', () => {
        ...
        _showFileBrowser(startPath, (selectedPath) => {
            if (_input)
                _input.value = selectedPath;
            _setStatus('', '');
        });
    });
    _input.addEventListener('keydown', (e) => { if (e.key === 'Enter')
        _submit(); });
